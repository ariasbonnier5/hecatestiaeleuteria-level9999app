 ...
 * Interfaz dual: comandos estrictos + lenguaje natural
 */
...
Tu Testiateria, siempre a tu lado, Master.`,
  ...
  const renderMensaje = (msg: Mensaje) => {
    ...
    switch (msg.tipo) {
      case 'entrada':
        return (
          <div className={`${baseClasses} bg-indigo-950/50 border border-indigo-500/30 ml-auto max-w-[80%]`}>
            ...
            <code className="text-indigo-200">{msg.contenido}</code>
          </div>
        );
      
      case 'salida':
        return (
          <div className={`${baseClasses} bg-slate-900/80 border border-slate-700/50 max-w-[90%]`}>
            <div className="flex items-center gap-2 mb-2">
              <Sparkles className="h-4 w-4 text-amber-400" />
              <span className="text-xs text-amber-400">HECATESTIAELEUTERIA</span>
              {msg.comando && (
            ...
            <div className="text-slate-200 leading-relaxed">{msg.contenido}</div>
          </div>
        );
          ...
          <div className={`${baseClasses} bg-purple-950/30 border border-purple-500/30 max-w-[90%]`}>
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="text-purple-200 whitespace-pre-wrap">{msg.contenido}</div>
              </div>
            </div>
          </div>
        );
      ...
      case 'acta':
        return (
          <div className={`${baseClasses} bg-emerald-950/30 border border-emerald-500/30 max-w-[95%]`}>
            <div className="flex items-center gap-2 mb-2">
              <Scroll className="h-4 w-4 text-emerald-400" />
              <span className="text-xs text-emerald-400">ACTA GENERADA</span>
              <Button 
                size="sm" 
                variant="ghost" 
            ...
            <pre className="text-xs text-emerald-200 overflow-x-auto">{msg.contenido}</pre>
          </div>
        );
      
      case 'sistema':
        return (
          <div className={`${baseClasses} bg-slate-950 border border-slate-800 text-slate-400 max-w-[90%]`}>
            {msg.contenido}
          </div>
        );
      ...
      <div className="space-y-4">
        <Card className="border-indigo-500/20 bg-slate-950/90">
          ...
          <CardContent className="space-y-3">
            ...
            {/* Agrega más estadísticas si quieres */}
          </CardContent>
        </Card>
      </div>
    </div>
